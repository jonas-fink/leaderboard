...
- [x] **BE-1** `config/index.ts`: `sessionSecret` als Pflicht-Env
...
- [x] **BE-2** `models/user.model.ts` mit `select: false` am `passwordHash`,
...
- [x] **BE-3** `auth.service.ts` umbauen — _test-first_, `auth.check.ts`
...
- [x] **BE-4** `services/session.service.ts` — `setSessionCookie`,
...
- [x] **BE-5** `middleware/auth.ts`: `requireUser` mit allen fünf Schritten aus
...
- [x] **BE-6** Besitz an den Modellen: `ownerId` an `Tournament` und `Player`,
...
- [x] **BE-7** `assertOwned` in `tournament.service.ts` (404, nicht 403) und
...
- [x] **BE-8** Die acht Löcher aus `architecture.md` § »Löcher« schließen:
...
- [x] **BE-9** `services/quota.service.ts` mit der `LIMITS`-Tabelle und
...
- [x] **BE-10** `GET /api/board/:userSlug/:tournamentSlug` — `boardBySlug` wird
...
- [x] **BE-11** `server/src/user-delete.ts` + `npm run user:delete`, gebaut nach
...
- [x] **BE-12** `seed.ts` legt einen Demo-User an und hängt das Beispielturnier
      samt Spielern daran.
      _done when:_ `npm --prefix server run seed -- --yes` läuft durch und der
      Demo-User kann sich anmelden.
      _needs:_ BE-11

- [x] **BE-13** Doku: `PROJEKT.md` §2 (Routen), §3 (`User`, `ownerId`), **§9
...
- [x] **BE-14** Besitzprüfung auf Fremdschlüsseln im Request-Body:
      `postScore`, `postMatch`, `postTeam` **und `patchTeam`** prüfen, dass
      jede übergebene `playerId` beziehungsweise `teamId` dem Konto aus
      `req.user` gehört, bevor sie an ein eigenes Turnier gehängt wird.
      `patchTeam` gilt ausdrücklich mit: `UpdateTeamSchema` lässt `members`
      durch, die Route ist derselbe Bug über ein anderes Verb.
      404 wie `assertOwned`, nicht 403. Deckt F-1 aus `qa-report.md` ab.
      _done when:_ Sitzung A kann keine `playerId` oder `teamId` von B in ein
      eigenes Turnier eintragen — der Versuch liefert 404; `npm --prefix
      server test` bleibt grün.
      _needs:_ BE-8

## Frontend

- [x] **FE-1** `lib/auth.ts` **löschen**; `lib/api.ts` ohne
...
- [x] **FE-2** `pages/Login.tsx` und `pages/Register.tsx` mit allen vier
...
- [x] **FE-3** `RequireSession`-Wrapper und Routen in `App.tsx`:
...
- [x] **FE-4** Durchsehen, dass keine Oberfläche mehr instanzweite Listen
